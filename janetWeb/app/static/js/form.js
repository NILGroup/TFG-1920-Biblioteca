	...
	$('form').on('submit', function(event) {
		...
		.done(function(data){
			...
			if (data.errorno == 0){
				...
				$('#messages').append("<div class='message inMessage'><p>" + data.response + "</p></div>");

				switch(data["content-type"]){
					case "list-books":
						$('#messages').append();
						data.books.forEach(function(element) {
							var htmlToApend = "<div class='message inMessage'>";
							htmlToApend += getBookHTML(element.title, element.author); 
							$('#messages').append(htmlToApend + "</div>");
						});
						break;
					...
					case "phone":
						$('#messages').append("<div class='message inMessage'><h4>" + data.library + ": " + data.phone + "</h4></div>");
						break;
				...
				$('#messages').append("<div class='message inMessage'><p>" + "No he podido realizar la consulta." + "</p></div>");
			}
			$('#submit').attr("disabled", false)


		});
		$('#messages').append("<div class='message outMessage'><p>" + $('#message').val() + "<p></div>")
		newMessageScroll();
...
function addMap(lat, long, location){
	...
	$('#messages').append("<div class='message inMessage'><h4>" + location + "</h4>" + 
			"<div class='map' id='" + mapid + "'></div></div>");
